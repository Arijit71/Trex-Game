...
function setup() {
  ...
  restart = createSprite(width/2,40+height/2);
...
function draw() {
  ...
  if (gameState===PLAY){
    ...
    if((keyDown("space")||touches.length>0) && trex.y >= ground.y-25) {
      trex.velocityY = -12;
      jumpSound.play();
      touches = [];
    }
...
function spawnClouds() {
  ...
  if (frameCount % 60 === 0) {
    ...
    cloud.y = Math.round(random((30/100)*height,(60/100)*height));
